...
function ChatBubbleSkeleton({ isCurrentUser }: { isCurrentUser?: boolean }) {
  return (
    <div
    ...
    >
      <div
        className={`
          ...
          ${isCurrentUser ? "bg-[#EDE3FD]" : "bg-[#EFF2F2]"}
        `}
      >
...
function ConversationSkeleton() {
  return (
    <>
      <ChatBubbleSkeleton />
      <ChatBubbleSkeleton isCurrentUser />
      <ChatBubbleSkeleton />
      <ChatBubbleSkeleton isCurrentUser />
      <ChatBubbleSkeleton />
    </>
...
const quickActions = [
 ...
 {isLoading ? 
  <ConversationSkeleton />: (messages?.map((msg) => {
    const isCurrentUser = msg?.user?.id % 2 === 0;
  ...
  <div className="flex items-center gap-4">
    {attachmentActions.map((icon, index) => (
      <Image
        key={index}
        src={icon.src}
        alt={icon.alt}
