...
import SingleChatPageMessages from "./ChatMessages/ConversationThread";
...
<div       style={{ backgroundColor: avatarAccentColor }}
 className="relative size-10 lg:size-[2vw] 2xl:size-10 rounded-full overflow-hidden  flex items-center justify-center text-white font-bold">
    ...
    <div className="w-full lg:w-auto">
      ...
      <p className={`transition-all duration-300 ease-in-out ${isSidebarVisible?"opacity-100":"opacity-0"} text-gray-500`}>
...
const { isSidebarVisible, toggleSidebar } = useSidebarStore();
 ...
 <DashboardSidebar openSidebar={isSidebarDrawerOpen}/>
    ...
    ${isSidebarVisible ? "lg:max-w-[20%] lg:opacity-100" : "lg:max-w-0 lg:opacity-0"} 
...
>
  ...
  <button
      onClick={toggleSidebar } // Use toggle to switch open/close
          ...
          <div className="px-[4%] pb-[4%]">
            {chats?.slice(0, 11).map((post: ChatPost, index: number) => (
              <ConversationPreviewCard
                key={post.id}
                post={post} isSidebarVisible={isSidebarVisible}
                isActive={index === 0}
